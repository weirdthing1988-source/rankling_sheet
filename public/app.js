...
function primaryStanceKey(){return stanceAvailable(state.formation.stance)?state.formation.stance:'';}
...
function renderTroopers(){
  ...
  state.troopers.forEach((t,index)=>{
    ...
    card.innerHTML=`
      ...
      <p class="small-note">${t.status==='formed'?`Shares the Formation pool as a ${segment.current}/${segment.max} HP chunk.`:t.status==='detached'?`Acts after ${escapeHtml(t.leader||'its Temporary Leader')}. ${orderCostText()}.`:t.status==='downed'?'Prone and Incapacitated until healed or recovered.':'Seeking shelter or the Brass’s last known position.'}</p>
      <div class="trooper-actions"><button type="button" data-act="strike">Trooper Strike</button><button type="button" data-act="damage">Damage</button><button type="button" data-act="heal">Heal</button><button type="button" data-act="return">Return</button></div>`;
...
function renderActions(){
  const str=mod(state.character.abilities.str),p=prof(),attack=str+p,profile=commandDiceProfile(),primary=primaryStanceKey(),formed=formedCount(),detached=state.troopers.find(t=>t.status==='detached');
  const actions=[];
  actions.push({name:"Brass's Spear",tag:'Attack action',text:`Make ${fighterAttacks()} spear attack${fighterAttacks()===1?'':'s'} when you take the Attack action. Trooper movements and sword strikes may describe these attacks without creating extra attack rolls.`,buttons:[{type:'roll',label:'Attack',formula:`1d20${fmt(attack)}`,desc:"Brass's Spear attack"},{type:'roll',label:'Damage',formula:`1d6${fmt(str)}`,desc:"Brass's Spear damage"}]});
  actions.push({name:'Trooper Strike',tag:'Detached Trooper',text:detached?`${detached.name} attacks with ${detached.weapon}. The fixed profile ignores weapon bonuses, mastery, feats and Fighting Styles; only the damage type changes to ${weaponDamageType(detached)}.`:'Detach one Trooper to enable its fixed short-weapon attack.',disabled:!detached,buttons:[{type:'roll',label:'Attack',formula:`1d20${fmt(attack)}`,desc:`${detached?.name||'Trooper'} Strike`},{type:'roll',label:'Damage',formula:`1d6${fmt(p)}`,desc:`${detached?.name||'Trooper'} Strike (${detached?weaponDamageType(detached):'weapon'}) damage`}]});
  actions.push({name:'Lock Shields',tag:'Shield Wall · Reaction',text:`Spend one Command Die to reduce damage to the cohort or an adjacent ally by the die + proficiency bonus. Shield Wall also grants +1 AC and advantage against being shoved or knocked Prone.`,disabled:primary!=='shield'||level()<3,buttons:[{type:'commandRoll',label:'Reduce',formula:profile.die?`1d${profile.die}${fmt(p)}`:'—',desc:'Lock Shields damage reduction'}]});
  actions.push({name:'Spearhead Charge',tag:'Spearhead · On hit',text:`After moving at least 10 feet directly toward the target, spend one Command Die to add it to spear damage. The target makes a Strength save against DC ${formationSaveDC()} or is pushed ${level()>=10?'10':'5'} feet.`,disabled:primary!=='spearhead'||formed<3||level()<3,buttons:[{type:'commandRoll',label:'Bonus damage',formula:profile.die?`1d${profile.die}`:'—',desc:'Spearhead Charge bonus damage'}]});
  actions.push({name:'Coordinated Blades',tag:'Assault Rank · Once per turn',text:`A melee hit deals +${level()>=10?p*2:p} damage from the Troopers’ coordinated blades. Spend one Command Die to also prevent the target from taking Reactions until the start of its next turn.`,disabled:primary!=='assault'||formed<3||level()<3,buttons:[{type:'roll',label:'Passive bonus',formula:String(level()>=10?p*2:p),desc:'Coordinated Blades bonus damage'},{type:'commandSpend',label:'Suppress Reactions',desc:'Suppressing Blows'}]});
  actions.push({name:'Guard the Ward',tag:'Escort · Reaction',text:`Ward: ${escapeHtml(state.formation.ward||'not selected')}. Impose disadvantage on one attack against the adjacent ward${level()>=10?' and grant that ward +1 AC while adjacent':''}. Spend one Command Die to move the ward up to half its speed without provoking Opportunity Attacks.`,disabled:primary!=='escort'||formed<2||level()<3,buttons:[{type:'commandSpend',label:'Guided Withdrawal',desc:'Guided Withdrawal'}]});
  if(level()>=7) actions.push({name:'Interpose',tag:'Delegated Command · Reaction',text:`A detached Trooper may move up to 10 feet to become adjacent to its Temporary Leader and reduce damage to that leader by ${p}, provided it can reach them.`,disabled:!detached,buttons:[{type:'roll',label:'Reduce',formula:String(p),desc:'Detached Trooper Interpose'}]});
  if(level()>=15) actions.push({name:'Emergency Recall',tag:'Instant Reformation · Reaction',text:'A detached Trooper may immediately move up to its speed toward the Brass when recalled. The Return button on its roster card rejoins it once it reaches formation.',disabled:!detached,buttons:[]});
  if(level()>=18) actions.push({name:'Trooper Sacrifice',tag:'Perfect Formation · 1/Long Rest',text:`When the cohort is reduced to 0 HP, one formed Trooper becomes Downed and the cohort remains at 1 HP. ${state.resources.lastStandAvailable?'Available.':'Already used this Long Rest.'}`,disabled:!state.resources.lastStandAvailable||state.character.currentHP>0||formed<1,buttons:[{type:'special',label:'Use at 0 HP'}]});
  ...
  actions.forEach(action=>{const card=document.createElement('div');card.className='action-card';if(action.disabled)card.style.opacity='.52';card.innerHTML=`<div class="action-top"><h3>${action.name}</h3><span class="action-tag">${action.tag}</span></div><p>${action.text}</p><div class="action-buttons"></div>`;const buttons=card.querySelector('.action-buttons');(action.buttons||[]).forEach(b=>{let btn;if(b.type==='commandRoll')btn=makeCommandRollButton(b.label,b.formula,b.desc,action.disabled);else if(b.type==='commandSpend')btn=makeCommandSpendButton(b.label,b.desc,action.disabled);else if(b.type==='special'){btn=document.createElement('button');btn.type='button';btn.textContent=b.label;btn.disabled=action.disabled;btn.addEventListener('click',useLastStand);}else btn=makeRollButton(b.label,b.formula,b.desc,action.disabled);buttons.appendChild(btn);});grid.appendChild(card);});
